...
getConst (BAEInteger _) = BAEUndefined
getConst (BAEFraction _) = BAEUndefined
getConst (BAESymbol _) = BAEInteger 1
getConst (BAESum _) = BAEInteger 1
getConst (BAEPower _ _) = BAEInteger 1
getConst (BAEFunction _ _) = BAEInteger 1
getConst (BAEProduct os)
    | isConstant (head os)  = head os
    | otherwise             = BAEInteger 1
...
simplifySum :: BAE -> BAE
simplifySum (BAESum os)
    | any isUndefined os        = BAEUndefined
    | length os == 1            = head os
    | otherwise                 = let v = simplifySum' (BAESum os) in if length v == 1 then head v else if null v then BAEInteger 0 else BAESum v

simplifySum' :: BAE -> [BAE]
simplifySum' (BAESum os)
    | length os == 2 && not (any isSum os)      = case1 (head os, os!!1)
    | length os == 2 && any isSum os            = case2 (head os, os!!1)
    | length os > 2                             = case3 os
    where
        case1 (e1, e2)
            | isConstant e1 && isConstant e2    = let p = simplifyRNE (BAESum [e1, e2]) in [p | p /= BAEInteger 0]
            | e1 == BAEInteger 0    = [e2]
            | e2 == BAEInteger 0    = [e1]
            | getTerm e1 == getTerm e2  = let
...
mergeSums p q
    ...
    | otherwise = let
        p1 = head p
        q1 = head q
        h = simplifySum' (BAESum [p1, q1])
        adjoin a b = a:b
        in if null h then mergeProducts (tail p) (tail q) else if length h == 1 then adjoin (head h) (mergeSums (tail p) (tail q)) else if h == [p1, q1] then adjoin p1 (mergeProducts (tail p) q) else adjoin q1 (mergeSums p (tail q))

simplifyUnary :: BAE -> BAE
simplifyUnary (BAEUnaryExpr c e)
    | c == '+'  = e
    | c == '-'  = simplifyProduct (BAEProduct [BAEInteger (-1), e])

simplifyDifference :: BAE -> BAE
simplifyDifference (BAEBinaryDiff a b) = simplifySum (BAESum [a, simplifyProduct (BAEProduct [BAEInteger (-1), b])])

simplifyQuotient :: BAE -> BAE
simplifyQuotient (BAEQuotient a b) = simplifyProduct (BAEProduct [a, b']) where b' = simplifyPower (BAEPower b (BAEInteger (-1)))

simplifyFunction :: BAE -> BAE
simplifyFunction (BAEFunction sym e) = if e == BAEUndefined then BAEUndefined else BAEFunction sym e
